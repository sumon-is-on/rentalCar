    ...
    <div class="flex ">
        <form action="{{ route('brand.index') }}" method="GET" role="search">
            <div class="flex input-group">
                ...
                <a href="{{ route('brand.index') }}" class="ml-2">
                    <button class="btn btn-info" type="button" title="Refresh page">Refresh</button>
                </a>
                <a href="{{ route('brand.create') }}" class="ml-2">
                    <button class="btn btn-info" type="button">Add Brand</button>
                </a>
            </div>
        </form>
    </div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    ...
                    <td class="text-sm text-gray-900 font-light px-6 py-1 whitespace-nowrap">
                        <img class="max-w-full h-auto rounded-full" src="{{ url('/brands/'.$data->image)}}" width="35px">
                    </td>
                    <td class="py-1 px-6 uppercase">{{ $data->country }}</td>
                    <td class="py-1 px-6 uppercase">{{ $data->details }}</td>
                    <td class="py-1 px-6">
                        <div class="flex">
                        <a href="{{ route('brand.show',$data->id) }}"class="text-indigo-600 hover:text-indigo-900">
                        ...
                        <a href="{{ route('brand.edit',$data->id) }}"class="text-indigo-600 hover:text-indigo-900">
                            <svg class="h-5 w-5 text-blue-600" viewBox="0 0 20 20" fill="currentColor">
                            <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z"/>
                            <path fill-rule="evenodd"d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z"clip-rule="evenodd"/>
                            </svg>
                        </a>
                        <a href="{{ route('brand.delete',$data->id) }}"class="text-indigo-600 hover:text-indigo-900" onclick="return confirm('Are you sure you want to delete it ?')">
